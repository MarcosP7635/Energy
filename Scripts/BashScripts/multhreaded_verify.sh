...
#that is a string of the file names
#CHATGPT explained this "Contains the newline-separated list of file paths all packed into one string."
#which is wrong, it's actually a [white]space separated list
readarray -t files_arr -d '' <<<  $files
echo "There are " ${#files_arr[@]} " files"
#that is an array of the file names
...
#
# Runtime of the sample cases is printed win times.log
#
...
function talys_run() {
  local index=$1
  local ii=$((index + 1))
  #The number of the file but 1-indexed unlike the variable index
  local dir=`dirname $files_arr[$index]`
  cd ${dir}
#
...
indices=($( seq 0 $((iimax-1)) ))
echo "Indices is: " ${indices[@]}
#YOU NEED THE AT that is the @ SYMBOL OR IT JUST USES THE FIRST ELEMENT!!!!!
